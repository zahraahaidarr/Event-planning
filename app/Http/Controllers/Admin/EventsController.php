...
public function store(Request $request)
{
    ...
    // 2) Build start/end datetime (you said these columns exist)
    ...
    // fallback: use first employee as "system" creator (so admins without employee row still work)
    if (!$creatorId) {
        $creatorId = Employee::min('employee_id');
    }

    if (!$creatorId) {
        // no employees at all -> nothing we can point to
        return response()->json([
            'ok'      => false,
    ...
    try {
        $event = DB::transaction(function () use ($validated, $category, $startsAt, $endsAt, $creatorId) {
